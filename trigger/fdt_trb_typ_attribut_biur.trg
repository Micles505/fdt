DROP TRIGGER FDT_TRB_TYP_ATTRIBUT_BIUR;

CREATE OR REPLACE TRIGGER FDT_TRB_TYP_ATTRIBUT_BIUR
 before insert or update on FDTT_TYP_ATTRIBUT
 for each row
begin
  case
    ...
    when inserting then
      ...
      if :new.no_seq_typ_attribut is null then
        :new.no_seq_typ_attribut := fdttattrb_seq.nextval;
      end if;
