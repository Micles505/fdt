create or replace package body fdt_pkb_apx_010101 is
  ...
  begin
    ...
    -- Si le nom de la date début de période est différente du an_mois_fdt, ça veut dire que l'utilisateur
    -- a sélectionné une autre période dans le select list de la page 2 (Pour corriger la fdt antérieure)
  ...
  begin
    ...
    if coalesce(vnu_no_seq_aact_intrv_det, 0) <> 0 then
      -- On vient ajuster la date début de période et find de période lorsque l'utilisateur est sur
      ...
      if pdt_dt_debut_periode = vda_date_debut_periode and pdt_dt_fin_periode = last_day(vda_date_debut_periode) then
         -- nous sommes dans l'onglet mois complet, il faut trouver la bonne période selon la date de l'absence.
         -- On va chercher le no_seq_ressource associé à la fdt.
  ...
  begin
    ...
    -- 
    -- On va chercher les dates de cette FDT où il n'y a aucune saisie.
  ...
  function f_obtenir_items_lov_intervention(pnu_no_seq_ressource         in fdtt_ressource.no_seq_ressource%type,
    ...
    cursor cur_obtenir_ressource_intervention is
      ...
      --
      union all
      -- Permet d'afficher les frais généraux non saisissable par l'utilisateur
