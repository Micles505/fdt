create or replace package body fdt_pkb_apx_010106 is
  -- ====================================================================================================
  -- Date        : 2022-05-10
  -- Par         : SHQYMR
  ...
  --
  cva_nom_collection constant varchar2(30) := 'COL_010106';
  --
  --
  ...
  begin
    ...
    apex_collection.add_member(p_collection_name => cva_nom_collection,
                               ...
                               p_n002            => pre_enreg.NO_SEQ_RESSOURCE,
    ...
    cursor cur_test_bd is
      select tinter.no_seq_temps_intrv,
       ...
       where tinter.no_seq_temps_intrv not in
         ...
         and tinter.no_seq_ressource = pnu_no_seq_ressource
  ...
  begin
    ...
    if not apex_collection.collection_exists(cva_nom_collection) Then
      ...
      for rec_temps_intervention in cur_tous_temps_intervention loop
        ...
        logger.append_param(p_params => vta_params,
                            p_name   => 'NO_SEQ_RESSOURCE',
                            p_val    => rec_temps_intervention.NO_SEQ_RESSOURCE);
        logger.append_param(p_params => vta_params,
 ...
 --                                                                            and cat.co_type_categorie <> 'GENRQ'
      ...
      from   fdtt_temps_intervention temp_inter
             inner join intervention inter      on  temp_inter.no_seq_aact_intrv_det = inter.no_seq_aact_intrv_det
                                                and inter.nombre_intervention_asso  > 1 
                                                and inter.no_seq_activite_decoupage = pnu_no_seq_activite_generaux
      where  temp_inter.no_seq_ressource = pnu_no_seq_ressource 
      and    temp_inter.debut_periode    = pdt_date_debut_periode 
      and    temp_inter.fin_periode      = pdt_date_fin_periode 
      --
      union all
      --
      select aid.description as d, 
             aid.no_seq_aact_intrv_det as r
      from   fdtt_temps_intervention ti 
             inner join fdtt_ass_act_intrv_det aid on aid.no_seq_aact_intrv_det = ti.no_seq_aact_intrv_det 
             inner join fdtt_intervention_detail interv on  interv.no_seq_intrv_detail = aid.no_seq_intrv_detail 
                                                        and (interv.code_intrv           = 'FGE' or 
                                                             interv.indic_visibilite_fdt = 'N')
      where  ti.no_seq_ressource = pnu_no_seq_ressource 
    ...
    --
    --
    vta_pipe_item_lov_intervention fdt_pkb_apx_010106.tab_pipe_item_lov_intervention;
    --
    -- logger
    --
  ...
  function f_obtenir_items_lov_periodes (pdt_date in date)
    ...
    --
    --
    vta_pipe_item_lov_periodes fdt_pkb_apx_010106.tab_pipe_item_lov_periodes;
    --
    -- logger
    --
